...
//
// Add the Leap Motion controller feature:
// 1) left hand to control the gesture, swipe, key-tap, cycle.
//      cycle type:  rotation
//      swipe type:  translation
//      key-tap:     scale, zoom in or out. 
// 2) right hand palm for hand movement, to zoom in, out, rotate and translate. 
// 
// author: Ruida Cheng
// 
...
public class DualDepthPeeling implements GLEventListener, KeyListener, MouseListener, MouseMotionListener
{
	...
	public static enum TYPE { ROTATE, TRANSLATE, SCALE};
	...
	public static void main(String[] args) {
		...
		animator.start();
			...
			final HandListener handListener = new HandListener(kWorld);
	...
	public void setParams(float g_oldX, float g_oldY, float g_oldZ, 
			              float g_newX, float g_newY, float g_newZ) {

		float rel_x = (g_newX - g_oldX) / (float)g_imageWidth;
		float rel_y = (g_newY - g_oldY) / (float)g_imageHeight;
		float rel_z = (g_newZ - g_oldZ) / (float)g_imageHeight;
		
		 if (Float.isNaN(rel_x) || Float.isNaN(rel_y) || Float.isNaN(rel_z))
		      return;
	...
	public void onFrame(Controller controller) {
		...
		for (Gesture gesture : frame.gestures()) {
			...
			if (lefthand.isLeft()) {
				switch (gesture.type()) {
				case TYPE_CIRCLE:
					System.err.println("set rotation");
					ddp.setRotate();
					break;
				case TYPE_SWIPE:
					System.err.println("set translation");
					ddp.setTranslate();
					break;
				case TYPE_KEY_TAP:
					System.err.println("set scale");
					ddp.setScale();
					break;
				default:
		...
		Vector rightHandCenter = rightHand.palmPosition();
		if (rightHand.isRight()) {
			g_oldX = g_newX;
			g_oldY = g_newY;
			g_oldZ = g_newZ;
		
			g_newX = rightHandCenter.getX();
			g_newY = rightHandCenter.getY();
			g_newZ = rightHandCenter.getZ();

			ddp.setParams(g_oldX, g_oldY, g_oldZ, g_newX, g_newY, g_newZ);
		}
	}
